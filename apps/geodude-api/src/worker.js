        ...
        try {
          ...
          try {
            if (env.CACHE) {
              ...
              rateLimitMetrics.ip_allow_5m = await metrics.getMetric("ip_rl_allow_5m");
              rateLimitMetrics.ip_block_5m = await metrics.getMetric("ip_rl_block_5m");

              // Calculate status
              if (rateLimitMetrics.ip_block_5m > 50 && rateLimitMetrics.ip_allow_5m / rateLimitMetrics.ip_block_5m < 5) {
                rateLimitMetrics.status = "degraded";
              } else if (rateLimitMetrics.ip_block_5m > 0) {
                rateLimitMetrics.status = "watch";
              } else {
                rateLimitMetrics.status = "healthy";
              }
            }
          } catch (e) {
            console.error("Failed to fetch rate limiting metrics:", e);
          }
