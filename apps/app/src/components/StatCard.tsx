...
export function StatCard({ label, value, sub, tone = "neutral" }: StatCardProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, color: colors.text, marginBottom: 6, opacity: 0.8, fontWeight: 500 }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 700, color: colors.text, lineHeight: 1 }}>
        {value ?? "—"}
      </div>
      {sub ? <div style={{ marginTop: 6, fontSize: 11, color: colors.text, opacity: 0.7 }}>{sub}</div> : null}
    </div>
  );
}
